...
public class GraphImprovePipelineIntegrationTest {
    ...
    public void testInit() throws PipelineStageException, MissingDataException {
        ...
        assertEquals(6, sentence1.getOutgoingArcs()
                                 .size());

        assertEquals(sentence2, sentence1.getOutgoingArcs()
                                         .get(0)
                                         .getTargetNode());
        assertEquals(18, sentence1.getIncomingArcs()
                                  .size());

        assertEquals(8, graph.getNodesOfType(graph.getNodeType("token"))
                             .stream()
                             .filter((node) -> ((String) node.getAttributeValue(
                                     GraphBuilder.CHUNK_IOB_ATTRIBUTE)).equals("B-NP"))
                             .count());
        graph.getNodesOfType(graph.getNodeType(PhraseEnum.NOUN_PHRASE.asNodeTypeName()))
             .forEach(node -> System.out.println(node.getIncomingArcs()
                                                     .size()));
    }
    ...
    public void checkEncapsulation() throws PipelineStageException, MissingDataException {
        ...
        Arrays.asList(PhraseEnum.values())
              .forEach((chunkIob -> {
                  ...
                  nodes.forEach((node) -> {
                      ...
                      int beginnerArcsSize = node.getIncomingArcsOfType(
                              graph.getArcType(PhraseHelper.BEGINN_TYPE_PREFIX + chunkIob.asNodeTypeName()))
                                                 .size();
                      int insideArcsSize = node.getIncomingArcsOfType(
                              graph.getArcType(PhraseHelper.INSIDE_TYPE_PREFIX + chunkIob.asNodeTypeName()))
                                               .size();
                      int sentence2phraseArcsSize = node.getIncomingArcsOfType(
                              graph.getArcType(SentenceToPhraseStrategy.SENTENCE2PHRASE))
                                                        .size();
                      ...
                      assertTrue(0 == (node.getIncomingArcs()
                                           .size()
                              - beginnerArcsSize - insideArcsSize - sentence2phraseArcsSize)); // No other incomming
                                                                                               // arcs
                  });
              }));
    }
